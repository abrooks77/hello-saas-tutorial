...
<html xml:lang="${cmsfn.language()}" lang="${cmsfn.language()}">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="${ctx.contextPath}/.resources/asciidoc/webresources/asciidoc.css" media="all" /> 
      [#-- <script src="${ctx.contextPath}/.resources/asciidoc/webresources/js/jquery.js"></script> --]
    [#-- or via theme --]
    ...
    [#-- uncomment next line to use resfn templating functions to load all css which matches a globbing pattern --]
      [#-- ${resfn.css(["/asciidoc/.*css"])!} --]
  </head>
  <body class="asciidoc-style ${cmsfn.language()}">

    <div class="container">
      <h1>asciidoc-style works!</h1>
      [@cms.area name="main"/]
    </div>

    [#-- use resfn to load all js which matches the globbing pattern or link resources manually or via theme --]
    [#-- ${resfn.js(["/asciidoc/.*js"])!} --]
  </body>
</html>
